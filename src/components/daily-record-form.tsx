...
export function DailyRecordForm({ userId }: { userId: string }) {
  ...
  const txtInp = "w-full bg-transparent text-[11px] outline-none focus:bg-blue-50 rounded h-7 text-left px-0.5"
        ...
        <table className="border-collapse text-[11px] w-max">
          <thead>
            <tr className="bg-gray-200 text-gray-700">
              ...
              <th className={`${c} w-16`}>食事</th>
              <th className={`${c} w-11`}>歩数</th>
              <th className={`${c} w-7`}>便</th>
              <th className={`${c} w-11`}>性状</th>
              <th className={`${c} w-7`}>酒</th>
              <th className={`${c} w-16`}>メモ</th>
            </tr>
          </thead>
          <tbody>
            {days.map((day, i) => {
              ...
              return (
                <tr key={day.date} className={isToday ? 'bg-blue-50' : 'bg-white'}>
                  {/* Date */}
                  <td className={`${c} sticky left-0 ${isToday ? 'bg-blue-100' : 'bg-white'} z-10 text-center font-medium leading-tight`}>
                    <span className="text-[11px]">{format(d, 'M/d')}</span>
                    <span className={`text-[10px] ml-0.5 ${isSun ? 'text-red-500' : isSat ? 'text-blue-500' : 'text-gray-400'}`}>{dow}</span>
                  </td>
                  ...
                  <td className={c}>
                    <select
                      ...
                      className="w-full bg-transparent text-[10px] outline-none focus:bg-blue-50 rounded h-7"
                    >
                  ...
                  {/* Memo (text) */}
                  <td className={c}>
                    <input value={day.memo} onChange={e => updateDay(i, 'memo', e.target.value)} className={txtInp} placeholder="-" />
                  </td>
                </tr>
              )
            })}
